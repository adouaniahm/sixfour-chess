...
//  SixFourChess
//
//  Encodes chess positions into numerical representations for the RAG module.
//
//  Responsibilities:
//    - Zobrist hash for transposition-table and opening-book lookups
//    - 768-dim feature vector for the CoreML NNUE model
//    - 96-byte compact embedding for SQLite storage
//
//  The implementation follows the Polyglot hashing convention and keeps
//  incremental updates cheap via XOR.
//
...
/// Encodes chess positions into Zobrist hashes and feature vectors.
///
/// Thread-safe via actor isolation. Stateless: can be shared between
/// multiple `RAGChessAI` instances.
actor BoardEncoder {
    ...
    /// Computes a Polyglot-compatible Zobrist hash for a position.
    ///
    /// The hash includes pieces, castling rights, en passant (if capturable), and side to move.
    /// Complexity: O(64) - full board traversal.
    ///
    /// - Parameter state: Board state, a Sendable value type.
    /// - Returns: 64-bit Zobrist hash. Same position always yields the same hash.
    nonisolated func zobristHash(for state: ChessState) -> UInt64 {
        var hash: UInt64 = 0

        // 1. XOR for every piece on the board.
        for row in 0..<8 {
            for col in 0..<8 {
                if let piece = state.board[row][col] {
                    let polyglotPiece = Self.polyglotPieceIndex(type: piece.type, color: piece.color)
                    let polyglotSquare = (7 - row) * 8 + col
                    let index = 64 * polyglotPiece + polyglotSquare
                    hash ^= Self.random64[index]
                }
            }
        }

        // 2. Castling rights.
        //    Polyglot: index 768 = white kingside, 769 = white queenside,
        //              770 = black kingside, 771 = black queenside
        //    Right present = king AND matching rook have not moved.
        if !state.isWhiteKingMoved && !state.isWhiteRookRightMoved {
            hash ^= Self.random64[768]  // White O-O
        }
        if !state.isWhiteKingMoved && !state.isWhiteRookLeftMoved {
            hash ^= Self.random64[769]  // White O-O-O
        }
        if !state.isBlackKingMoved && !state.isBlackRookRightMoved {
            hash ^= Self.random64[770]  // Black O-O
        }
        if !state.isBlackKingMoved && !state.isBlackRookLeftMoved {
            hash ^= Self.random64[771]  // Black O-O-O
        }

        // 3. En passant, only if an opposing pawn can actually capture.
        //    Polyglot: index 772 + file (0=a, 7=h)
        if let ep = state.enPassantTarget {
            if canEnPassantBeCaptured(at: ep, in: state) {
                hash ^= Self.random64[772 + ep.col]
            }
        }

        // 4. Side to move.
        //    Polyglot: XOR when it is White to move.
        if state.currentPlayer == .white {
            hash ^= Self.random64[780]
        }

        return hash
    }

    /// Computes a 768-dim feature vector for the NNUE input.
    ///
    /// Format: 12 planes × 64 squares.
    /// - Planes 0-5  : white pieces (pawn, knight, bishop, rook, queen, king)
    /// - Planes 6-11 : black pieces
    /// - In each plane: row-major, row 0 = rank 8
    ///
    /// Note: this format differs from Polyglot, which uses `kind*2+color`.
    /// For NNUE, pieces are grouped by color to make training easier.
    ///
    /// - Parameter state: Board state.
    /// - Returns: 768 floats (0.0 or 1.0), ready for `MLMultiArray`.
    nonisolated func featureVector(for state: ChessState) -> [Float] {
        var features = [Float](repeating: 0.0, count: 768)

        for row in 0..<8 {
            for col in 0..<8 {
                if let piece = state.board[row][col] {
                    let channel = Self.nnueChannelIndex(type: piece.type, color: piece.color)
                    let squareIndex = row * 8 + col
                    features[channel * 64 + squareIndex] = 1.0
                }
            }
        }
    ...
    /// Compresses the feature vector into 96 bytes (768 bits).
    ///
    /// Used to store embeddings in the SQLite database.
    /// Each bit corresponds to a feature-vector entry.
    /// Order: bit 0 = features[0], bit 7 = features[7], byte 0 = features[0..7].
    ///
    /// - Parameter state: Board state.
    /// - Returns: 96 bytes (packed 768 bits, little-endian per byte).
    nonisolated func compactEmbedding(for state: ChessState) -> Data {
        let features = featureVector(for: state)
    ...
    /// Checks whether an opposing pawn can actually capture en passant.
    ///
    /// Needed because Polyglot only includes the en passant hash when the capture
    /// is actually possible, meaning an adjacent opposing pawn exists.
    nonisolated private func canEnPassantBeCaptured(at epSquare: Position, in state: ChessState) -> Bool {
        let capturingColor = state.currentPlayer
        // The capturing pawn is on the row adjacent to the en passant square.
        let pawnRow = capturingColor == .white ? epSquare.row + 1 : epSquare.row - 1

        for colOffset in [-1, 1] {
            let col = epSquare.col + colOffset
            guard col >= 0, col < 8, pawnRow >= 0, pawnRow < 8 else { continue }
            if let piece = state.board[pawnRow][col],
               piece.type == .pawn,
    ...
    /// Polyglot piece index: `kind * 2 + color`.
    /// kind: 0=pawn, 1=knight, 2=bishop, 3=rook, 4=queen, 5=king
    /// color: 0=black, 1=white
    private static func polyglotPieceIndex(type: PieceType, color: PieceColor) -> Int {
        let kind: Int
        switch type {
        case .pawn:   kind = 0
        case .knight: kind = 1
        ...
        let colorBit = color == .white ? 1 : 0
        return kind * 2 + colorBit
    }

    /// NNUE index grouped by color.
    /// White: pawn=0, knight=1, ..., king=5
    /// Black: pawn=6, knight=7, ..., king=11
    private static func nnueChannelIndex(type: PieceType, color: PieceColor) -> Int {
        let base = color == .white ? 0 : 6
        switch type {
        case .pawn:   return base + 0
...
// MARK: - Constantes Polyglot Random64

extension BoardEncoder {

    /// The 781 official Zobrist constants from the Polyglot specification.
    ///
    /// Structure:
    /// - [0..767]   : 12 piece types × 64 squares (`piece = kind*2+color`, `square = rank*8+file`)
    /// - [768..771] : castling rights (WK, WQ, BK, BQ)
    /// - [772..779] : en passant file (a..h)
    /// - [780]      : side to move
    ///
    /// Source: PolyGlot 1.4w by Fabien Letouzey (`hash.cpp`) / python-chess.
    /// Check: `hash(initial_position, white_to_move) == 0x463b96181691fc9c` ✓
    ///
    /// These values match those used by all Polyglot `.bin` opening books.
    /// Do not modify them, or compatibility with existing `.bin` files will break.
    // swiftlint:disable:next line_length
    static let random64: [UInt64] = [
